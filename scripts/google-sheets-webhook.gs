...
function doGet() {
  return json({ ok: true, service: "BankConnector", action: "Use POST { action: 'fill', columns: {...} } or run fillBalancesSheet from the sheet." });
}

function doPost(e) {
  const body = e && e.postData ? JSON.parse(e.postData.contents) : {};
  if (body.action === "fill") {
    const result = fillBalancesSheetImpl_(body);
    return json(result);
  }
